...
unemployment <-
  here("Slides/05_Time_Series/data/FRED/UNRATE.csv") |>
  read_csv(show_col_types = FALSE) |>
  janitor::clean_names() |>
  filter(date >= date("1976-01-01"))

recessions <-
  here("Slides/05_Time_Series/data/FRED/USREC.csv") |>
  read_csv(show_col_types = FALSE) |>
  janitor::clean_names() |>
  filter(date >= date("1976-01-01"))
...
# %%
kfbmisc::tikzsave(
  here("Slides/05_Time_Series/figures/unemployment_rate.pdf"),
  plot = p_unemployment,
  width = 8,
  height = 4.5
)
kfbmisc::tikzsave(
  here("Slides/05_Time_Series/figures/unemployment_rate_rand_shuffle.pdf"),
