...
\item $\kelly{\varepsilon_i}$ could be positive or negative if we over/under-estimate $\coral{\hat{Y}_i}$. If we add up all the residuals, the positive and negatives will sometimes cancel out (same reasoning as variance). This isn't good because we want to be 'penalized' for all prediction errors. 
  
--

2. Therefore we minimize $\kelly{\varepsilon_i}^2$ because each $\kelly{\varepsilon_i}^2$ will be positive.

\item In general,*the line of best-fit* means the line that minimizes the .kelly[sum of squared errors] (SSE):
