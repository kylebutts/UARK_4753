---
title: "Lab 06 - Time-Series Data and Time-Series Regression"
author: "YOUR NAME"
...
This part will focus on time-series regression. 
For this, we will use the unemployment data as well as a data on the "movie and concert ticket price index". 

## Unemployment rate

### Estimating seasonality patterns

Let's see which quarter of the year has the lowest unemployment rate.
We can create this using the `quarter` function from lubridate:
...
est <- feols(
  unrate ~ i(quarter(date)),
  data = unemployment,
  vcov = "hc1"
)
etable(est_bad_version, est)
```

Just like with cross-sectional regression, a regression of an outcome on a set of indicator variables with an intercept estimates difference in means relative to the baseline (omitted) group.
Without an intercept (using `0 + ` in our formula) estimates the different group means: 

```{r}
est_without_intercept <- feols(
  unrate ~ 0 + i(quarter(date)),
  data = unemployment,
  vcov = "hc1"
)
etable(est, est_without_intercept)
```


### Estimating linear trend

Let's fit a simple linear time-trend to the data. 
Since `date` is just a numeric variable that increments by 1 for each day, we can just include it in our regression model:
```{r}
...
Let's combine both a linear time-trend and indicators for each quarter in a single regression.

```{r}
unemployment_model <- feols(
  unrate ~ date + i(quarter(date)),
  data = unemployment
)
unemployment_model
```

The coefficient on `date` represents the linear time-trend (every day unemployment rate goes down on average by -0.000124 percentage points).
The coefficients on `quarter` represent a quarter specific seasonal effect; there appears to be no statistically significant seasonal pattern (since each coefficient is insignificant)



## Movie and Concert Price Ticket

### Time-series Regression Model

Now let's load our movie and concert price index dataset. 
...
df <- read.csv("data/movie_and_concert_ticket_price_index.csv")
...
We are going to fit a model with a linear time-trend and indicators for each month for the movie and concert price index data. 
To do so, first create a variable called month in your dataset using the `month(x, label = TRUE)` function from `lubridate`. 
Then regress the price index on the month using `feols` with the formula `price_index ~ date + i(month)`:

```{r}
# Create a variable called `month` using the `month()` from `lubridate`

# Regress the price index on `date` and month indicators
...
1. In your regression, what is the omitted month?
...
4. Interpret the linear trend coefficient in words and form a 95% confidence interval for this estimate.

ANSWER HERE 

5. Which parts of the data does our linear trend do well? Which parts do we predict poorly? 
...
# Regress the price index on month indicators and the piecewise-linear trend
